...
`dialogues/` 内の雑談対話JSONファイルから、Gemini APIを用いて対話の **主題（theme）** と **状況サマリ（summary）** を抽出します。  
...
.
├── README.md                       # 本ファイル
├── large_train.jsonl               # 訓練データ（大）
├── small_train.jsonl               # 訓練データ（小）
├── labeled_test_dataset.jsonl      # テスト用ラベル付きデータ
├── train_detection.jsonl           # 皮肉検出訓練用
├── create_detaset.py               # 検出用データセット作成スクリプト
├── make_test_detaset.py            # テストデータセット作成スクリプト
├── file_counter.py                 # ファイルカウントユーティリティ
├── sample/                         # サンプルデータ
│   ├── sarcasm_dataset_sample.jsonl
│   └── sarcasm_dataset_readable_sample.txt
└── make-sarcasm/                   # 皮肉データ生成パイプライン
...
`dialogues/` ディレクトリに、ソースとなる雑談対話のJSONファイルを配置します。  
...
- `dialogues/` ディレクトリの元対話データは本リポジトリには含まれていません（`.gitignore` で除外）。元データのライセンスに従ってご利用ください。
